...
freq_min       = 600; % best for CA1, 300 best for cortex/pfc
freq_max       = 6000;
mask_artifacts = 1;
samplerate     = 30000;
threshold      = 5; % for artifact masking
chunk_size     = 2000; % for artifact masking
tet_option     = []; % struct to increase verbosity of messages (see parent function ml_process_animal)
...
if exist(filt_out,'file') && ~overwrite
    ...
    % Bandpass filter
    if ~isempty(tet_option)
        tetOptStr = sprintf('...tetrode = %d, area = %s', tet_option.tetrode, tet_option.area);
    else
        tetOptStr = '';
    end
    fprintf('\n------\nFiltering, Maksing and Whitening for  %s\n\nBandpass Filtering. Parameters:\n\tfreq_min: %g\n\tfreq_max: %g\n\tsamplerate: %g %s\n------\n',tetResDir,freq_min,freq_max,samplerate,tetOptStr)
