function [out,sortedFilenames] = ry_getTrodesCommentsStrings(rawDir,sortedFilenames, varargin)
% Ryan fixes ... if fails to find a given *.videoTimeStamps, say if one epoch
% is lacking them, it should switch to the alternate strategy. But the standard
% chooses useVideo if even a single videoTimestatmps is found. 
...
if useVideo
    try
        ...
        for i=1:Nfiles
            ...
            pauses = find(diff(timeStamps)>=pauseDetectionGap);
            if ~isempty(pauses)
        ...
        disp("UseVideo failed.. trying with *.time")
        disp(ME)
        useVideo = false;
    end
...
if ~useVideo
    ...
    epochs = getEpochs(pauseDetectionGap);
