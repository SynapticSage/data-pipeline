function [newpos] = addvelocity(posdata, filt, varargin)
...
%RN minor edits to sj_addvelocity (renaming functions used due to matlab shadowing) 8-4-17
% assign default values to varargin options
ip = inputParser;
ip.addParameter('posfilt', []);
ip.parse(varargin{:})
Opt = ip.Results;
...
indzeropos = find((pos(:,2) == 0) | isnan(pos(:,2)));
...
if ~isempty(Opt.posfilt)
    filtlength = length(Opt.posfilt);
    % now iterate through every 'good' chunk of pos and smooth it
    for i = goodpos
        if i(2)-i(1) > 3*filtlength
            pos(i(1):i(2),:) = [filtfilt(Opt.posfilt, 1, pos(i(1):i(2),1)) ...
                                filtfilt(Opt.posfilt, 1, pos(i(1):i(2),2))];
        end
    end
end
...
vel(2:end) = euc_dist(pos(1:end-1,:), pos(2:end,:)) ./ frametime;
vel(1) = vel(2);
...
indzerovel = indzerovel(ismember(indzerovel, 1:length(time)));
vel(indzerovel) = 0;
...
if fieldcol     % this pos struct already contains 'vel', so replace the existing vel
    ...
    % then replace Opt.posfilt - sj: nothing to replace if using Steve's method
    %filtrow = find(strcmp(newpos.arg(:,1), 'Opt.posfilt'));
    %newpos.arg{filtrow, 2} = Opt.posfilt;
    newpos.arg=[];
    newpos.arg{end+1,1} = 'Opt.posfilt';
    newpos.arg{end,2} = Opt.posfilt;
else    % this is the first time vel has been computed for this pos structure, so add
    % put value of 'Opt.posfilt' in the newpos.arg field
    newpos.arg=[];
    newpos.arg{end+1,1} = 'Opt.posfilt';
    newpos.arg{end,2} = Opt.posfilt;
