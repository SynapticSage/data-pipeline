function generateSPWRevents(directoryname, fileprefix, day, tetrode, ...
        min_suprathresh_duration, nstd, varargin)
...
%
% --------------------------
% Ryan, pulled this from sj_extractripples
%
% modified to allow pulling from rippleref
...
pos = ndb.load(fileprefix, 'pos', 'inds', d);
...
for t = progress(tet,'Title','tetrode')
    match = sprintf('*%s%02d-*-%02d.mat', rippletype, day, t);
    tmpflist = dir(match);
    assert(~isempty(tmpflist));
    for i = progress(1:length(tmpflist),'Title','epoch')
        % load the ripple file
        tmpflist(i).name;
        tmp = load(tmpflist(i).name);
        ripple = tmp.(rippletype);
        % get the epoch number
        dash = find(tmpflist(i).name == '-');
        e = str2double(tmpflist(i).name((dash(1)+1):(dash(2)-1)));
        
        
        % Check if ripple band exists - may not be true for reference(Shantanu-Jun2013)       
        if isfield(ripple{d}{e}{t},'data') ...
                && isfield(ndb.get(pos, [d,e]), 'data') && ~isempty(pos{d}{e}.data)
