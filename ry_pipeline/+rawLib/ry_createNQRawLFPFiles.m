function ry_createNQRawLFPFiles(rawDir, dataDir, animID, sessionNum)
...
for datFileInd = 1:length(datFiles)
    ...
    for e = 1:size(epochList,1)
        ...
        raw{currentSession}{e}{nTrodeNum}.low_pass_filter = data.low_pass_filter; % mcz from Binary2FF_LFP.m
        raw{currentSession}{e}{nTrodeNum}.voltage_scaling = data.voltage_scaling; % mcz from Binary2FF_LFP.m

        save(rawFile,'-v6','raw');
    end
end
save('ME_rawDirect','ME');
cd(currDir);
