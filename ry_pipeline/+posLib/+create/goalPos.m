...
egopos.description = ["gridTable : table of grid positions and their properties - some measurements are computed against this", "stopWell : well an animal ends a trajectory on", "startWell : well an animal starts a trajectory on", "goalVec : goal vectors to each well in complex coordinates", "velVec : heading vector defined by velocity", "headVec : dheading-like vector defined by head direction", "egoVec : the egocentric vector using velocity heading in complex coordinates", "angle (egoAngle) : the egocentric angle", "euclidianDistance : the euclidean distance of the animal to each well", "headEgoAngle : the head-direction based egocentric angle", "currentPathLength : path length to the current goal", "pathX : x of animal on path", "pathY : y on animal on path", "trialTime : start/stop times of a current path at every time point", "currentAngle : current egocentric angle", "currentEucDist : current euclidean distance", "currentGoalVec : current vector to the current goal", "currentEgoVec : current egocentric vector to the goal, using velocity heading", "currentHeadEgoAngle : current egocentric vector to the goal, using head direction as heading", "currentGoalVec : current vector to goal", "currentHeadEgoAngle : current head direction based ego vec" , "cuemem : the cue-memory state", "region : the region that the animal is currently inside"];
egopos.description = join(egopos.description, newline);
