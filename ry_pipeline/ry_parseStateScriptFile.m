function events = ry_parseStateScriptFile(fileName, varargin)
...
%line in the file.
% RYan Y - updated to provide flexibility in number of input output bits to watch
ip = inputParser;
ip.addOptional('timeConvert', [], @isnumeric)
ip.addOptional('inputWatch',  [], @isnumeric)
ip.addParameter('numBits',    32, @isnumeric); % Number of IO bits to examine
ip.parse(varargin{:})
opt = ip.Results;
        ...
        while 1
            ...
            if isempty(tline)
                ...
                if (~isempty(tmpTime) && (length(tokCell)>1)&& (~isequal(tokCell{2},'RFsync'))) %only parse lines that start with the time stamp
                    ...
                    if ~isempty(opt.timeConvert)
                        tmpTime = interp1(opt.timeConvert(:,1),opt.timeConvert(:,2),tmpTime,'linear','extrap');
                    end
                    events(numEvents).time         = tmpTime;
                    events(numEvents).inStateDiff  = zeros(1, opt.numBits);
                    events(numEvents).outStateDiff = zeros(1, opt.numBits);
                    ...
                    if ((length(tokCell)==3) && (~isempty(str2num(tokCell{2}))) && (~isempty(str2num(tokCell{3}))))
                        events(numEvents).inState = bitget(str2num(tokCell{2}),1:opt.numBits); %get the binary input state of all opt.numBits ports
                        ...
                        events(numEvents).outState = bitget(str2num(tokCell{3}),1:opt.numBits); %get the binary output state of all opt.numBits ports
...
%relavent events
if (~isempty(opt.inputWatch))
    tmpEvents = events;
    events = [];
    eventNum = 0;
    for i = 1:length(tmpEvents)
        if ((~isempty(tmpEvents(i).message)) || (sum(abs(tmpEvents(i).inStateDiff(opt.inputWatch)))>0) || (sum(abs(tmpEvents(i).outStateDiff))>0))
            
            %tmpInStateDiff = tmpEvents(i).inStateDiff(opt.inputWatch);
            eventNum = eventNum+1;
